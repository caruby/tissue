...
EOS
    ...
    #
    # @quirk caTissue A collected Specimen must have a label. The caTissue API does not
    #   auto-generate a label unless an auto-generator is defined. The work-around is to set
    #   the label to a unique value when the status is updated to collected.
    #
    ...
    def update_object(obj)
      ...
      elsif CaTissue::SpecimenEventParameters === obj then
        return update_from_template(obj)
    ...
    def prepare_specimen_for_update(obj)
      ...
      if obj.collection_status == 'Collected' and  obj.label.nil? then
        obj.label = Jinx::UID.generate.to_s
    ...
    def create_object(obj)
      if CaTissue::Address === obj then
        ...
        if obj.parent and obj.characteristics == obj.parent.characteristics then
          if obj.label.nil? then
            obj.label = obj.barcode = Jinx::UID.generate.to_s
      ...
      obj.take_snapshot if Annotation === obj and obj.changed?
    ...
    # @quirk JRuby Fetching a specimen association can replace the specimen consent tier status
    #   with a different, empty status. The status swizzle occurs even though the attribute fetched
    ...
    def fetch_participant_using_ppi(pnt)
      ...
      pnts = query_toxic(cpr.copy, :participant)
      pnts.first if pnts.size == 1
    end
    ...
    def fetch_participant_using_mrn(pnt)
      ...
      pnts = query_toxic(pmi.copy(:site, :medical_record_number), :participant)
      pnts.first if pnts.size == 1
    end
    ...
    def fetch_specimen_alternative(spc)
      ...
      spcs = query_toxic(eid.copy, :specimen)
      spcs.first if spcs.size == 1
    end
