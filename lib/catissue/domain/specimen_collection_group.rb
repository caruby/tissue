...
module CaTissue
  ...
  class SpecimenCollectionGroup
    ...
    add_attribute_aliases(:collection_event => :collection_protocol_event,
      ...
      :specimen_events => :specimen_event_parameters
    )
    
    # @quirk caTissue 2.0 The SCG event parameters are moved to SOP dynamic extensions.
    #   Access is mediated by a new dependent +action_applications+ property.
    #
    # @quirk caTissue 2.0 The new SCG +action_applications+ reference is cascaded, but
    #   
    ...
    # Define the SCG pathology annotation. Use the presence of the action_applications reference to
    # distinguish a caTissue 2.0 from a pre-2.0 release, since the annotation package changes in 2.0. 
    if property_defined?(:action_applications) then
      # @quirk caTissue 2.0 The SCG pathology annotation package changed from +pathology_scg+
      #   to +gov.nih.nci.dynext.deintegration+.
      #
      # @quirk caTissue 2.0 The +SCGRecordEntry+ annotation proxy package changed from +pathology_scg+
      #   to +gov.nih.nci.dynext.pathology_scg+.
      add_annotation('Pathology', :packages => ['gov.nih.nci.dynext.pathology_scg'], :proxy_name => 'SCGRecordEntry')
    else
      add_annotation('Pathology', :packages => ['pathology_scg'], :service => 'pathologySCG', :proxy_name => 'SCGRecordEntry')
    end
    
    # @quirk caTissue Bug #64: Initialize the SCG +consent_tier_statuses+ to an empty set.
    #
    # @quirk caTissue 2.0  Initialize the SCG +action_applications+ to an empty set.
    def initialize
      super
    ...
    def saved_attributes_to_fetch(operation)
      attrs = super
      attrs += self.class.collectible_event_parameter_attributes if operation.type == :update
      attrs
    end

    private
    
    # The collectible event attribute to fetch, as follows:
    # * +action_applications+ for caTissue 2.0 or greater,
    # * +specimen_event_parameters+, otherwise.
    def self.collectible_event_parameter_attributes
      @cep_attrs ||= property_defined?(:action_applications) ? [:action_applications] : [:specimen_event_parameters]
    end
    ...
    # * Sets the auto-generated action applications owner to this SCG, if necessary.
    #
    ...
    def add_defaults_local
      ...
      if self.class.property_defined?(:action_applications) then
        action_applications.each { |aa| aa.specimen_collection_group ||= self }
      end
    ...
    def create_default_received_event_parameters
      ...
      # make the REP
      ev = CaTissue::Collectible.create_parameters(:received, self, :user => rcv)
    ...
    def create_default_collection_event_parameters
      ...
      ev = CaTissue::Collectible.create_parameters(:collection, self, :user => rep.user, :timestamp => rep.timestamp)
      ev.add_defaults_recursive
    ...
    def validate_event_parameters
      if identifier.nil? and collectible_event_parameters.empty? then
