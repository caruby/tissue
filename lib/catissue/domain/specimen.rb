...
module CaTissue
  ...
  class Specimen
    ...
    #   Initialize the Specimen consent_tier_statuses if necessary.
    #
    ...
    # @quirk caTissue 2.0 Some Specimen event parameters are moved to SOP dynamic extensions.
    #   Access is mediated by a new dependent +action_applications+ property.
    add_dependent_attribute(:action_applications) if property_defined?(:action_applications)
    ...
    # @quirk caTissue 2.0 the Specimen parent_changed flag is discontinued in caTissue 2.0.
    #
    # @quirk caTissue the Specimen parent_changed flag is ignored in a caCORE update
    #   or create Specimen argument and is set on the server. Remove this pre-2.0 property
    #   that has no place in a client API.
    remove_attribute(:parent_changed) if property_defined?(:parent_changed)
    ...
    #   caRuby marks the +specimen_events+ as auto-generated and fetch on save. However,
    #   SpecimenRequirement events are not auto-generated. Therefore, the AbstractSpecimen
    #   events behavior must be qualified for Specimen but not SpecimenRequirement.
    if property_defined?(:action_applications) then
      qualify_attribute(:specimen_events, :autogenerated, :fetch_saved)
    end
    
    # Define the Specimen pathology annotation.
    #
    # @quirk caTissue 2.0 The +SpecimenRecordEntry+ annotation proxy package changed from +pathology_specimen+
    #   to +edu.wustl.catissuecore.domain.deintegration+.
    #
    # @quirk caTissue 2.0 The Specimen pathology annotation package changed from +pathology_specimen+
    #   to +gov.nih.nci.dynext.pathology_specimen+.
    if property_defined?(:action_applications) then
      add_annotation('Pathology', :packages => ['gov.nih.nci.dynext.pathology_specimen', 'edu.wustl.catissuecore.domain.deintegration'])
    else
      add_annotation('Pathology', :packages => ['pathology_specimen', 'edu.wustl.catissuecore.domain.deintegration'], :service => 'pathologySpecimen')
    end
    ...
    # @quirk caTissue Bug #64: Initialize the Specimen +consent_tier_statuses+ to an empty set.
    #
    # @quirk caTissue 2.0 Initialize the Specimen +action_applications+ to an empty set.
    def initialize
      super
      self.consent_tier_statuses ||= Java::JavaUtil::LinkedHashSet.new
      if self.class.property_defined?(:action_applications) then
        self.action_applications ||= Java::JavaUtil::LinkedHashSet.new
      end
    end
...
#    def recover
#      # TODO - test this
#      dep = specimen_events.detect { |ep| CaTissue::DisposalEventParameters === ep }
#      return if dep.nil?
#      dep.delete
#      self.available = true
...
#    end
    ...
    def disposed?
      specimen_events.any? { |ep| CaTissue::DisposalEventParameters === ep }
    end
    ...
    def dispose(reason=nil)
      dsp = CaTissue::DisposalEventParameters.new(:specimen => self, :reason => reason)
    end
    ...
    def validate_local
      ...
      if collected? then
        ...
        if received_event_parameters.nil? then
    ...
    # * Sets the auto-generated action applications owner to this specimen, if necessary.
    #
    ...
    def add_defaults_local
      super
      # Fix the auto-generarted action applications.
      if self.class.property_defined?(:action_applications) then
        action_applications.each { |aa| aa.specimen ||= self }
      end
    ...
    def create_default_received_event_parameters
      ...
      rep = CaTissue::Collectible.create_parameters(:received, self, :user => user)
    ...
    def create_default_collection_event_parameters(rep)
      ...
      cep = CaTissue::Collectible.create_parameters(:collection, self, :user => user)
      logger.debug { "Created default #{qp} collection event parameters #{cep.qp}." }
      cep
    end
